...
bool
mprinttupleslot(TupleTableSlot *slot)
{
	TupleDesc	tupdesc = slot->tts_tupleDescriptor;
	int			i;
...
IndexScanState* findIndexScanStateM(PlanState* node){
	...
	retval = findIndexScanStateM(outerPlanState(node));
	if (retval)
		return retval;
	
	return findIndexScanStateM(innerPlanState(node));
}

// Dfs of plan tree for plan node of IndexScan
IndexScan* findIndexScanNodeM(Plan* node){
	...
	retval = findIndexScanNodeM(outerPlan(node));
	if (retval)
		return retval;
	
	return findIndexScanNodeM(innerPlan(node));
}

// find the sql plan which contains the indexscan node
// return NULL if not found
PlannedStmt* extractIndexScanNodeM(char* query){
	...
	foreach(lc, stmt_list){
		...
		if ( (node = findIndexScanNodeM(stmt->planTree)) != NULL ){
			plan = stmt;
...
TupleDesc copyAndDropLastAttrM(TupleDesc tupdesc, bool doDrop){
...
FaginsState* InitFaginsState(Relation relation, uint32_t k, uint32_t term_cond, std::vector<IndexScanState*> scans, std::vector<QueryDesc*> qDescs){
	...
	state->ps.ps_ResultTupleDesc = copyAndDropLastAttrM(scans[0]->ss.ps.ps_ResultTupleDesc, scans.size()>1);
	state->ps.ps_ResultTupleSlot = MakeTupleTableSlot(state->ps.ps_ResultTupleDesc, scans[0]->ss.ps.ps_ResultTupleSlot->tts_ops);

	state->ps.ExecProcNode		 = execFaginsM;
...
Datum multicol_topk(PG_FUNCTION_ARGS) {
	...
	if (SRF_IS_FIRSTCALL()) {
		...
		for ( std::string exp : orderby_expressions) {
			...
			if ( !(plan = extractIndexScanNodeM(sourceText)) ){
			...
			IndexScanState* node = findIndexScanStateM(queryDesc->planstate);

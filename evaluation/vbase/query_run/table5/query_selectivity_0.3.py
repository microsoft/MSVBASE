...
def query_generate(path_img, path_or):
    outputfile="/artifacts/vbase/query_run/table5/sql/query_selectivity_0.3_vbase.sql"
    ...
    with open(path_img, 'r', encoding="utf8") as f_img, \
        ...
        for (id, image_embedding), (_, popularity_count) in zip(tsv_img, tsv_or):
            ...
            out.write("\\timing on\n")
            out.write(f"select id from recipe_table where (popularity<={3000}) order by image_embedding<*>ARRAY[{image_embedding}] limit 50;\n")
            out.write("\\timing off\n")
            idx += 1
            if idx%1000 == 0:
